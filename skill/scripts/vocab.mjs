...
const harvest = () => {
  ...
  const buttons = uniq([...document.querySelectorAll("button, [role='button'], a[href]")].filter(vis).filter((el) => !el.closest("nav, aside, header")).map(labelOf).filter((t) => t && t.length < 60));
  ...
  return { title: document.title, nav, headings, buttons, dialogs, tooltips };
};

const screens = [];
for (const url of urls) {
  await page.goto(url, { waitUntil: "load", timeout: 60000 }).catch((e) => console.error(`${url}: ${e.message}`));
